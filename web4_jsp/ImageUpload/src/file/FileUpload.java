...
public class FileUpload extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		//Users/sunjukim/Bitcamp_web/web4_jsp/.metadata/.plugins/org.eclipse.wst.server.core/tmp0/wtpwebapps/ImageUpload/upload

		//2가지 방법이 있음
		//1. tomcat에 배포(Server)	-> 자료가 사라질 가능성 있음()
		//임시 서버 경로로 나온다
//		String fupload = req.getSession().getServletContext().getRealPath("/upload/");
		String fupload = req.getSession().getServletContext().getRealPath("/ababab/test");
		System.out.println("file upload dir: " + fupload);
...
//				System.out.println("파일이름 바꿔랑");
		...
		if(b) {
			...
			RequestDispatcher dis = req.getRequestDispatcher("./uploadAf.jsp");
			dis.forward(req, resp);
//			resp.sendRedirect("./uploadAf.jsp?filename="+filename);
		}
		else {
